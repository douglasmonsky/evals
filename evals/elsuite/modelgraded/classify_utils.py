...
def get_choice_strings(choice_strings: Union[List[str], str], n: Optional[int] = None) -> List[str]:
...
def get_choice(
    ...
    if match:
        model_choice = ''.join(char for char in match.group(1) if char not in string.punctuation)
        for choice in choice_strings:
            if match_fn(model_choice, choice):
                logging.debug(f"Matched {model_choice} to {choice} for {eval_type}: {text}")
                return choice
